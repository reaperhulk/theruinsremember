...
After viewport emulation, the test opens and closes Preferences with native focused keyboard input and waits for each state transition. The summary is included in the pointer hit-test assertions alongside all volume controls; ordinary pointer activation is still exercised by the earlier desktop volume checks. This final disclosure-test refinement has the same unchanged 16-row persona comparison.
