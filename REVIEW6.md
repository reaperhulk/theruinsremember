...
## Browser verification refinements

The first CI run passed actual first-click output, pause/resume, volume, mute, and preference reload checks, then timed out waiting for a background tab. The old headless-shell executable does not apply the full browser's tab visibility behavior. The experience suite now installs and uses full Chrome, consistent with [Puppeteer's documented distinction between the two modes](https://pptr.dev/guides/headless-modes).

Full Chrome passed 82 experience checks. All ten rendered tracks measured RMS 0.049–0.055 at the default volume, with peak amplitude below 0.244. The live player peaked at 18 simultaneous voices including effects; there were no page errors. Screenshot review found that the initial responsive-panel check had accepted closed Preferences content, so subsequent refinements require the actual open state, nonzero dimensions, and pointer hit-test access to the summary and every control. The test waits for rendered frames after a viewport change and checks native keyboard opening/closing as well as the earlier pointer-driven volume controls.

Detailed hit diagnostics identified the blocker: returning from the background-audio check presents the game's normal offline progress report. A click aimed at the header was dismissing that overlay. The test now acknowledges the report through its visible Continue button before exercising Preferences; it retains every open-panel and reachability assertion. The wrapping header also explicitly allows visible overflow so its menus can extend beyond the old mobile scrolling strip.

Each verification refinement and the header overflow change used the verified post-audio snapshot as its pre-change baseline. The separate post-change comparisons for full Chrome, open-panel hit testing, overflow, resize settling, keyboard disclosure input, and return-report handling all produced exactly the same 16 before/after rows above. No engine files or checked-in calibration baselines changed.
