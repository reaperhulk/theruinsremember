...
export const achievements = [
  ...
  { id: 'starNetworkFull', name: 'Omniversal Web', description: 'Maintain 10 star routes through a prestige cycle', check: s => (s.starRoutes?.length || 0) >= 10 && (s.prestigeCount || 0) >= 1, reward: 20 },
  ...
  // --- 6 new achievements ---
  { id: 'upgradeVeteran180', name: 'Upgrade Veteran', description: 'Purchase 180 upgrades', check: s => Object.keys(s.upgrades || {}).length >= 180, reward: 20 },
  ...
  { id: 'allEra3', name: 'Era 3 Complete', description: 'Buy all Era 3 gateway upgrades', check: s => ['internet','digitalSensors','cloudComputing','openSource','aiResearch'].every(id => s.upgrades?.[id]), reward: 3 },
  { id: 'allEra4', name: 'Era 4 Complete', description: 'Buy all Era 4 gateway upgrades', check: s => ['rocketScience','spaceStation','solarArrays','zeroGManufacturing','nuclearReactor'].every(id => s.upgrades?.[id]), reward: 4 },
  { id: 'allEra5', name: 'Era 5 Complete', description: 'Buy all Era 5 gateway upgrades', check: s => ['asteroidMining','outerColony','terraforming','fusionPower','antimatterDrive'].every(id => s.upgrades?.[id]), reward: 5 },
  { id: 'chainReaction', name: 'Chain Reaction', description: 'Complete all gateway upgrades in eras 1 through 5', check: s => ['tools','irrigation','basicPower','housing','foundry','storehouse','assemblyLines','steelForge','ironWorks','powerGrid','computingLab','internet','digitalSensors','cloudComputing','openSource','aiResearch','rocketScience','spaceStation','solarArrays','zeroGManufacturing','nuclearReactor','asteroidMining','outerColony','terraforming','fusionPower','antimatterDrive'].every(id => s.upgrades?.[id]), reward: 20 },
  ...
  { id: 'oneHour', name: 'Getting Started', description: 'Play for 45 minutes total', check: s => s.totalTime >= 2700, reward: 1 },
  ...
  { id: 'starMapper', name: 'Star Mapper', description: 'Create 6 star routes', check: s => (s.starRoutes?.length || 0) >= 6, reward: 3 },
  { id: 'colonyEmpire50', name: 'Colony Governor', description: 'Assign 50+ colonies', check: s => { const a = s.colonyAssignments || {}; return (a.growth || 0) + (a.science || 0) + (a.industry || 0) >= 50; }, reward: 5 },
  { id: 'allMiniGamesV2', name: 'All Mini-Games', description: 'Have activity in all 7 mini-games', check: s => (s.totalGems || 0) > 0 && (s.hackSuccesses || 0) > 0 && (s.dockingPerfects || 0) > 0 && (s.totalWeaves || 0) > 0 && (s.starRoutes?.length || 0) > 0 && Object.values(s.colonyAssignments || {}).some(v => v > 0) && Object.values(s.factoryAllocation || {}).some(v => v > 0), reward: 10 },
