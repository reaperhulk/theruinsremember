...
export function GoalsPanel({ state, onUpdate }) {
  ...
  return <details className="panel goals-panel">
    ...
    {state.goals.length > 0 && <ol>{state.goals.map((item, index) => <li key={`${item.kind}:${item.id}`}>{options.find(o => o.value === `${item.kind}:${item.id}`)?.name || item.id} <button onClick={() => onUpdate(s => removeGoal(s, index))}>Cancel</button></li>)}</ol>}
