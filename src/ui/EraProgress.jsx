...
export function EraProgress({ state, objective = getEraObjective(state), onNavigate, onUpdate }) {
  ...
  return <section className="panel era-panel objective-panel" aria-label="Current objective" style={{ '--chapter-color': chapter.color }}>
    ...
    <div className="objective-main"><div><h2>{objective.title}</h2><p className="era-hint">{objective.detail}</p></div>
      {objective.stage === 'ready' && needsEraReview(state) && onUpdate ? <button className="era-advance-btn" onClick={() => onUpdate(approveEraAdvance)}>Continue to {eraNames[state.era + 1]} →</button> : state.autoBuildOut === false && objective.target && !objective.target.queued && onUpdate && canAfford(state, objective.target.cost) ? <button className="objective-purchase" onClick={() => onUpdate(s => (objective.target.kind === 'project' ? purchaseProject : objective.target.kind === 'tech' ? unlockTech : purchaseUpgrade)(s, objective.target.id))}>{objective.target.kind === 'tech' ? 'Research' : 'Build'} {objective.target.name}</button> : onNavigate && <button className="objective-link" onClick={() => onNavigate(objective.destination)}>{objective.stage === 'ready' && state.era === 10 ? 'Review inheritance' : objective.destination === 'mini' ? 'Open operation' : objective.destination === 'tech' ? 'Research choices' : 'Set priorities'} <span aria-hidden="true">↗</span></button>}
    </div>
    <div className="objective-meters">
      <label>{cycle ? 'Inheritance' : 'Foundation'} <span>{cycle ? (cycle.ready ? 'Ready' : `${cycle.completed}/${cycle.total}`) : `${Math.min(readiness.foundationProgress, readiness.minUpgrades)}/${readiness.minUpgrades}`}</span><progress aria-label={cycle ? 'Cycle readiness' : 'Era foundation progress'} value={cycle ? (cycle.ready ? cycle.total : cycle.completed) : Math.min(readiness.foundationProgress, readiness.minUpgrades)} max={cycle ? cycle.total : readiness.minUpgrades} /></label>
