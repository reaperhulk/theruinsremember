...
export function PublicWorksPanel({ state, onUpdate }) {
  ...
  return <section className="panel public-works-panel" aria-label="Economic route">
    ...
    <p>{work.complete ? 'Economic mastery complete. Earlier resources produce ×5 this era; construction supplies part of the era foundation.' : `${formatNumber(work.delivered)} / ${formatNumber(work.cost)} ${resources[work.resource].name}. Supplies 20% of new production${state.archive.research.salvage || state.archive.projects.continuityGarden >= 4 ? ' and reclaims overflow' : ''}; stronger industry finishes sooner.${income > 0 ? ` About ${formatTime(Math.ceil((work.cost - work.delivered) / income))} remaining.` : ''}`}</p>
