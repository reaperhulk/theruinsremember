...
export function PurchaseGuidance({ state, onUpdate, economy: suppliedEconomy }) {
  ...
  const automated = state.autoBuildOut !== false && target.automated && !target.queued;
  ...
  return <section className="panel purchase-guidance" aria-label="Purchase guidance">
    <strong>{target.queued ? 'Your commitment' : automated ? 'Automatic development' : 'Next purchase'}: {target.name}</strong>
    <p>{pressure.length === 0 ? (automated ? 'Funded; the team will complete this project automatically.' : 'Affordable now.') : Number.isFinite(pressure[0].eta) ? `About ${formatTime(Math.ceil(pressure[0].eta))} at current net income.` : 'Waiting alone will not fund this purchase at current production and storage.'}</p>
