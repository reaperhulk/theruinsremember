...
const asTarget = (state, def, kind) => def && ({ kind, id: def.id, name: def.name,
  cost: kind === 'tech' ? def.cost : getUpgradeCost(state, def.id), automated: kind === 'tech' ? !isDecisionTech(def) : !isDecisionUpgrade(def) });
...
export function getEraObjective(state) {
  ...
  const availableUpgrades = getAvailableUpgrades(state).filter(u => !u.repeatable && (state.autoBuildOut === false || !isDecisionUpgrade(u)));
  ...
  if (cycle) {
    ...
    detail = minimum ? `${minimum} more projects establish the base. Workshops build them as resources arrive.`
