...
function commissionInterval(state) {
  return DYSON_COMMISSION_INTERVAL * (state.upgrades?.forkOrbitalYards ? 0.8 : 1) * (state.prestigeUpgrades?.perfectMemory ? 0.5 : 1) * (state.archive?.projects?.relayNetwork >= 2 ? 0.5 : 1);
}
